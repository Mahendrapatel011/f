...
import { HiLocationMarker, HiShoppingCart, HiHome, HiUserCircle, HiX, HiSearch } from 'react-icons/hi';
import { MdMyLocation } from 'react-icons/md';
import { useLocationContext } from '../../context/LocationContext';
import { useCart } from '../../context/CartContext';

// ─── Mock Data (Real app mein API se aayega) ───────────────────────────────
const POPULAR_CITIES = [
    { city: 'Bhopal', area: 'MP Nagar', pincode: '462011' },
    { city: 'Indore', area: 'Vijay Nagar', pincode: '452001' },
...
const LocationDropdown = ({ onClose, onSelect }) => {
    ...
    return (
        <div className="absolute top-full -left-[75px] sm:left-0 mt-2 w-[300px] sm:w-[320px] bg-white rounded-xl shadow-2xl border border-gray-100 z-50 overflow-hidden">
            ...
            <div className="p-4 space-y-4">
                ...
                <div className="space-y-2">
                    <div className="flex gap-2">
                        ...
                        <button
                            ...
                            className="bg-[#1e3a5f] text-white px-4 rounded-lg text-sm font-medium hover:bg-[#2d5a8e] transition-colors flex items-center gap-1"
                        >
                            <HiSearch size={16} />
                        </button>
                    </div>
                ...
                {/* Popular Cities */}
                <div>
                    <p className="text-xs text-gray-400 font-medium mb-2 uppercase tracking-wide">Available Areas</p>
...
const LoggedInNavbar = () => {
    ...
    // Outside click se dropdown close ho
    useEffect(() => {
    ...
    const handleLocationSelect = (newLocation) => {
        updateLocation({ ...userLocation, ...newLocation });
    };
            ...
            <div className="max-w-[1150px] mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex items-center justify-between h-[80px] md:h-[90px]">
                    ...
                    <div className="flex items-center gap-3 sm:gap-8">
                        ...
                        <Link to="/listing" className="flex items-center shrink-0">
                            <img
                                ...
                                className="w-[80px] md:w-32 h-[45px] md:h-28 object-contain"
                            />
                        </Link>
                        ...
                        <div className="relative" ref={dropdownRef}>
                            <button
                                ...
                                className={`flex items-center gap-1 md:gap-2 border rounded-lg pl-1 pr-2 py-1 md:px-3 md:py-1.5 bg-white hover:bg-gray-50 transition-all cursor-pointer min-w-[130px] md:min-w-[180px] ${showLocationDropdown ? 'border-[#1e3a5f] ring-1 ring-[#1e3a5f]' : 'border-gray-200'}`}
                            >
                                <span className="text-[#e23744]">
                                    <HiLocationMarker className="w-4 h-4 md:w-5 md:h-5" />
                                </span>
                                <div className="flex flex-col text-left flex-1">
                                    <span className="text-[9px] md:text-[10px] text-gray-500 font-medium leading-none truncate max-w-[80px] md:max-w-none">
                                        {userLocation.area}, {userLocation.city}
                                    </span>
                                    <span className="text-[10px] md:text-[11px] text-[#1e3a5f] font-bold leading-tight">
                                        {userLocation.pincode}
                                    </span>
                                </div>
                                {/* Chevron */}
                                <svg
                                    className={`w-3 h-3 text-gray-400 transition-transform hidden md:block ${showLocationDropdown ? 'rotate-180' : ''}`}
                    ...
                    <div className="flex items-center gap-3 md:gap-8">
                        <Link to="/cart" className="relative text-[#1e3a5f] hover:text-[#2d5a8e] transition-colors p-1 md:p-2">
                            <HiShoppingCart className="w-6 h-6 md:w-7 md:h-7" />
                            {cart?.items?.length > 0 && (
                                <span className="absolute -top-1 right-0 md:top-1 md:right-1 w-4 h-4 bg-[#e23744] text-white text-[10px] flex items-center justify-center rounded-full font-bold shadow-sm">
                                    {cart.items.length}
                                </span>
                            )}
                        </Link>
                        ...
                        {showHomeIcon ? (
                            <Link to="/listing" className="text-[#1e3a5f] hover:text-[#2d5a8e] transition-colors p-1">
                                <HiHome className="w-7 h-7 md:w-9 md:h-9" />
                            </Link>
                        ) : (
